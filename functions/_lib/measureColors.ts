...
// Household measuring-tool colours (migration 0048). The nine editable swatches —
// six physical spoons (qty|unit) + three per-unit fallbacks — set in Réglages ▸
// Affichage and read back by every recipe surface so the measure pills + Cook-mode
// scoop circles match the household's own spoons everywhere. Validation lives here
// so the read path and the PATCH share one definition of "a valid swatch / colour".
// Keep this id list in sync with MEASURE_SWATCHES in src/lib/measurePrefs.ts.

const TOOL_KEYS = ['1|tbsp', '1/2|tbsp', '1|tsp', '1/2|tsp', '1/4|tsp', '1/8|tsp'] as const
