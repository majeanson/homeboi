...
| Help registries | `src/lib/{board,liste,routines,kitchenTab,operator,add,cercle,notes}Help.ts` | 8 maps of `{ body, card, point? }` — every hint names its guide card/point. Maison merges two of them (`{...CERCLE_HELP, ...ROUTINES_HELP}`) since one page now hosts both worlds. **A registry is only alive if some surface renders a `HelpToggle`**: `OPERATOR_HELP`'s entries were unreachable until 2026-09-09 because Réglages had none — and the moment it got one, five of its most-used cards turned out to render no `HelpTitle` at all (`helpKey` alone is the `?focus=` ANCHOR; a bubble needs `help` too). Written + wired 2026-09-10; `operatorHelpCoverage.test.ts` now makes a silent card declare itself ANCHOR_ONLY with a reason. |
| Tours | `src/lib/tour.tsx`, `src/lib/tourContent.ts`, `components/tour/TourOverlay.tsx` | Spotlight walkthroughs; steps anchor `data-tour` keys, can end on a guide card. `guidePoint(id, frLabel)` reuses any guide point's detail verbatim as a step body (`guidePlusActions(id)` = the « ＋ » point). A step with `sheet: true` walks INSIDE the ＋ sheet — HubLayout holds the section chooser open for it (in-sheet anchors: `add-note`, `add-tiles`, `add-week`, `add-routines`). A step with **`route`** navigates on ENTERING it (forward and backward), which is what lets one tour cross sections. |
| **Quick tour or grand tour** | `sectionChain()` + the `branch` step in `src/lib/tourContent.ts` | The welcome card ASKS: « Suivant » keeps the 30-second orientation (`essentials`, board only), « Voir les six sections » switches to `grand`, which walks **all six hub tabs** in the canonical nav order. Both auto-launch first-run and both replay from Réglages ▸ Découvrir ▸ « Première fois ». A ~40-step walk is right for someone settling in and wrong for someone who just wants their list, so the app asks instead of guessing (2026-09-10). Branching marks the tour you LEAVE as seen (`branchTo`), or someone who deliberately took the long tour would be greeted by the same welcome card next load. It is **built from the section tours, never rewritten**: their steps are already single-sourced from their guide cards, so adding a step to « La cuisine » adds it to the grand tour for free and the two can't drift. Only the six TABS are chained — « Routines » and « Le cercle » are sections inside Maison, and including them would double the length to show places you reach from a tab you were just shown; they keep their own « ? » offer. A missing tour id throws at module load rather than silently shipping a shorter tour. Guard: `e2e/tour-nav.spec.ts` walks it end to end and asserts every hub route is visited (proven red by removing the `route` navigation). |
