...
function Block({ block, onAction }: { block: DetailBlock; onAction: (a: DetailAction) => void }) {
  switch (block.kind) {
    case 'text':
        ...
        <div className="detail-sheet__listwrap">
          ...
          <ul className="detail-sheet__list">
            {block.items.map((it, i) => {
              // A plain string stays plain text. A DetailListRow adds the small glyph
              // doors (a day peek's meal: 📖 recette · 🍲 Cuisiner) — same runAction
              // path as the footer, so the peek closes then navigates.
              ...
              return (
                <li key={i} className="detail-sheet__listrow">
                  {/* The flex lives on an INNER span: flexing the <li> itself drops its
                      display:list-item, so a door-row would silently lose the bullet
                      that every plain row keeps. */}
