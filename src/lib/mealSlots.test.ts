...
describe('meal slot ordering', () => {
  it('lists slots by time of day: déjeuner → dîner → collation → souper → dessert', () => {
    expect(SLOT_TIME_ORDER).toEqual(['breakfast', 'lunch', 'snack', 'supper', 'dessert'])
  })

  it('ranks slots strictly increasing in time order (snack before supper, dessert last)', () => {
    ...
    expect(SLOT_RANK.supper).toBeLessThan(SLOT_RANK.dessert)
  })

  it('side slots are every slot but supper, in time order', () => {
    expect(SIDE_SLOTS).toEqual(['breakfast', 'lunch', 'snack', 'dessert'])
  })
