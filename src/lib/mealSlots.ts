...
// and a dessert (after the souper). `supper` stays the primary slot — the board
// headline, the kid suggestion target, the shop-the-week driver — so it's rendered
// prominently; the others are lighter "also planned" slots. A slot can hold
// SEVERAL meals (migration 0033).
...
// souper, dessert. (Distinct from MEAL_SLOTS, which keeps supper before snack for
// the "supper is primary" grid layout.) Use this anywhere meals are listed in
// order, and mirror it in the SQL CASE in meals.ts / board.ts / month.ts.
export const SLOT_TIME_ORDER: MealSlot[] = ['breakfast', 'lunch', 'snack', 'supper', 'dessert']

// Rank for sorting a mixed list of meals by time. Keep in sync with the SQL CASE.
export const SLOT_RANK: Record<MealSlot, number> = { breakfast: 0, lunch: 1, snack: 2, supper: 3, dessert: 4 }

// The "side" slots shown beside the souper, in time order.
