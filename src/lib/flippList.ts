...
//     on a phone): a Babillard payload there → ONE question, « Coller dans Flipp ? »
//     OK = REPLACE the Flipp list with this one (what is only in Flipp is removed),
//     Annuler = ADD to it — so « Vider » then « Coller » is one run, not two
//     (Marc, 2026-09-11: « can 1 and 2 be streamlined »); a CLEAR payload (« Vider ma
//     liste Flipp » on the till) → its own confirm, naming what is lost; anything else,
//     or no clipboard access → the prompt. (The way back used to ride this confirm's
//     Annuler; it lives in the prompt now — copy any word first if a Babillard list is
