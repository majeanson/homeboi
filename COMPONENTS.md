...
| **ToddlerCookBook** | `components/kitchen/ToddlerCookBook.tsx` | #45 (reframed) — the toddler's ON-SCREEN picture cookbook: a swipeable book (cover + one big page per recipe, photo/picto + name read aloud in the recipe's language) shown inside `KidKitchen` via a "📖 Mon livre" door; a big "On cuisine !" hands off to the toddler cook stepper. Reuses recipes + useSpeak + pictoFor; no route (lives on the kid surface). |
| **RecipeBook** | `components/kitchen/RecipeBook.tsx` | #45 — the printable toddler recipe/activity book (PARENT). Now entered from the kitchen ＋ ▸ "Faire un livre" (navigate-only AddSheet `book` mode → `/kitchen/book`), NOT an on-page button. A `/kitchen/book` scene (entered from `RecipesTab` ▸ "Faire un livre"): a cover + one page per recipe (big sticker photo, tick-box ingredients, numbered step bubbles with step photos). Reuses recipes + the tag/collection layer; `window.print()` + `styles/book.css` `@media print` paginate one recipe per sheet (no migration, no endpoint, no PDF lib). |
