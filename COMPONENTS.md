...
| **DrawPad** | `components/DrawPad.tsx` | The family draw pad for a fridge note (#14) — useful + educational. Four tools — freehand **pen** (`signature_pad`), tap-to-stamp **sticker packs** (faces/animals/nature/seasons/things/letters), chunky **pixel** grid (faint snap-grid overlay), and a **text** stamp (type a word) — plus a **mirror/kaleidoscope** toggle, **undo + redo**, a family rainbow + **custom/recent colours**, 3 sizes, paper-colour eraser. A **template** layer sits under the drawing: ruled **handwriting lines**, **letter/number tracing** (A–Z/a–z/0–9), **dot grid**, or a **colour-in** outline (star/heart/flower/house/fish/sun). Exports a **size-capped** PNG (`MAX_EDGE` 1280). **Share** (Web Share API / download fallback) and, via `onMakeRoutine`, **turn the drawing into a kid-routine card** (`lib/drawingToRoutine`). Pass `initial` to re-open + **add to** a drawing (shared doodle; Notes ✏️ → `PATCH /api/notes`); `toddler` trims to the big, safe controls. Full-screen overlay; `useModal` + `touch-action/user-select:none` lock the page. **Drawings show only in the board's Grille/bento view** (`Notes variant="drawings"`); other notes ride every view. |
