...
| **DrawPad** | `components/DrawPad.tsx` | Quick paint canvas for a drawn fridge note (#14). Three tools — freehand **pen** (`signature_pad`), tap-to-stamp **stickers**, chunky **pixel** grid (with a faint snap-grid overlay) — plus a family rainbow palette, 3 brush/cell sizes, **undo**, paper-colour eraser. Exports a **size-capped** PNG (`MAX_EDGE` 1280, so a drawing can't bloat to multi-MB and jank the board). **Share** (Web Share API / download fallback) and, via `onMakeRoutine`, **turn the drawing into a kid-routine card** (`lib/drawingToRoutine` → re-uploads to `routine-card-photo`, opens the routine builder seeded). Pass `initial` (an image URL) to re-open a drawing and **add to it** (shared family doodle; Notes ✏️ → `PATCH /api/notes`). Full-screen overlay; `useModal` + `touch-action/user-select:none` lock the page so a stroke can't swipe or select text behind it. |
