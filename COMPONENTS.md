...
| 5 | **Modal / sheet / scene** ⏸ | 12+ overlays | **DEFERRED — needs a dedicated effort, not a sweep.** The "overlays" are heterogeneous: RecipeSheet/RecipeForm/CookMode are now scene **routes**, ZoomableImg is a lightbox, EntityCombobox a dropdown, DrawPad a full editor — only ProfilePicker/KidExitGate/AddSheet/confirm are true sheets. Unifying these onto `Modal`+`Sheet` is real architecture (focus-trap, scroll-lock, swipe-dismiss, mount strategy) with high regression risk; do it deliberately with per-overlay verification, not at the tail of a sweep. |
