...
| **DayHeroes** | `components/board/DayHeroes.tsx` | The board's two "today" hero cards: the « Ce soir » supper card (every supper as a tappable row) + the weather card with the daily-wonder photo as its backdrop (`now-card--wx-photo`). The meal tap is a caller callback (`onOpenMeal`) so the host keeps its own detail actions; the wonder/shuffle is passed in. The weather card also carries a calm **few-hours-ahead strip** (`hours`: 3 icon+temp chips, `.now-card__hours`, frosted over the photo) from `/api/weather`'s `hours` (Open-Meteo `hourly`). Rendered by the **Grille** board view (`Board.tsx`). Returns null when there's neither a supper nor weather. `.board-heroes` / `.now-card*` in `styles/board.css` + `today.css`. |
| **Fil** / **placeFil** | `components/board/Fil.tsx`, `lib/dayRibbon.ts` | « Le fil du jour » — the day-ribbon: today's TIMED events read as a SHAPE (in time order, spaced by a clamped proportional gap = a soft time axis, past ones dimmed, a calm « maintenant » divider between past and upcoming); all-day items pool under a « À tout moment » `SubHead`. Rows reuse the board's own `eventAct` (tap → the same detail peek); a per-minute tick drifts the marker. An optional grid card (`lib/boardCards` `'fil'`, default before `today`); shown only with ≥2 timed events — a lone next-up is already the `Prochainement` headline, which hides when the fil is on screen. Toddler lens reuses `DayTimeline` (« Notre journée »). Pure layout in **`placeFil`** (`lib/dayRibbon`, unit-tested). `.fil*` in `styles/board.css`. Calm: shows position, never a count or an alarm. |
