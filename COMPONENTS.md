...
| **CercleNotes** | `components/cercle/CercleNotes.tsx` | « Le cercle » → its own **Notes** sub-tab (the third **SubTabs** section beside Famille/Social) → "Notes & recommandations": iOS-Notes-style quick notes scoped to one member (the "Moi" list) or the whole Maisonnée (family-wide), with optional media (audio/drawing/photo). The **shared pick-a-face control** (the same one as the board's "Aujourd'hui" header, surface-for-surface via `useSurface` — **MemberSwitcher** row on a kiosk, **FaceSelect** chip on mobile; seeded from `useProfile` but picks locally) sets whose notes show **and** the new note's scope: a member sees their own **plus** the Maisonnée's and writes a personal note; "Maisonnée" sees only family-wide and writes a family-wide one (`lib/familyNotes` `visibleNotes`). No separate Moi/Maisonnée toggle — scope follows the picked face. Composer = **EditField** (multiline) + **MemoControls** (reused via its `endpoint`/`affectedKey`/`extraBody` props → `/api/family-notes`). CRUD via **useWrite** + **useDeferredRemoval**; cards reuse the `.note-card` look + **ZoomableImg**/**DrawPad**. Backed by `family_notes` (migration 0062). Searchable in `/search`. |
...
| **DaySection** / **useDayWindow** | `components/board/DaySection.tsx`, `lib/useDayWindow.ts` | THE one rendering of "a day's agenda": meals → events → chores → home as the shared `Act` rows, each tapping the entity-detail peek, with optional `TodoSection` (« À compléter ») + `DayNote`. Reads a `DayItems` slice from **`useDayWindow(from,to)`** — the single place the `/api/month` window read + per-day bucketing lives (shared by Moments/Mois/Départ/La journée instead of each re-rolling it). Adapters absorb the month data shapes (event `at` not `start_at`, chore `who` is a name). `eveningOnly` does the « ce soir » trim. Built standalone (the old views still hand-roll their rows — they'll migrate in the later collapse). |
| **LaJournee** | `components/board/LaJournee.tsx` | « La journée » — the unified **prototype** board view (toggle key `jour`, `lib/boardview`), surfaced first in `BoardViewToggle`. A **face lens** (`MemberSwitcher`+`useProfile`: Maisonnée or a person filters everything) + scope `SubTabs`: **Maintenant** (the next-up focus + « À régler » via `useARegler`) and **Aujourd'hui** (the day through `DaySection`, with « Demain » behind a `Disclosure` — curated). Reuses `useDayWindow`/`DaySection`/`Section`/`AutoCard`/`CercleBirthdays`/weather. The old 5 views stay untouched as reference; this is the thing we iterate toward the eventual collapse. `.lajournee` family in `styles/board.css`. |
