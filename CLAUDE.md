...
| **Discovery (comprendre ↔ agir)** | The one guide/help/tour ↔ feature/settings mapping: **34** guide cards today against a **~32 merge-first ceiling** (over by two — merge before you add a 35th), each a launcher (« Ouvrir »/« Régler »/« Essayer ») as well as an explanation; "?" hints + tours deep-link back via `?card=&point=`; `?focus=` lands on a stacked settings card, `?plus=` opens the ＋ sheet; `SUB_GOTO` links a Réglages sub to its live surface. **See `DISCOVERY.md`** — taxonomy, URL grammar, invariants, checklists. |
