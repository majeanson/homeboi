...
| **Fridge memos**             | A fridge note (`notes`, board `Notes.tsx`) can carry an R2 attachment: an **audio memo** (#38) or a **drawn note** (#14 — a quick paint canvas, `DrawPad` over `signature_pad`). Both are created in the ＋ "Note rapide" sheet (`MemoControls`), general-audience, tinted by the active face; clearing frees the R2 blob. Migration 0043 (`media_kind`/`media_key`); upload via `note-media`. R2 unset → the controls hide, text notes still work. |
