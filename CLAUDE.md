...
| **Cook mode**                | Full-screen recipe view; follows the audience profile; exit via small ✕. `CookMode`. Each ingredient carries **« Il en manque »** — the same pantry-low write the Garde-manger uses, so noticing you are out mid-recipe costs no trip off the scene (parent + non-guest; a flagged row checks and disables). |
