...
| A surface that is still loading | **`Skeleton`** if the reader already knows the SHAPE (rows, a grid of tiles) — it reserves the space so nothing jumps under a thumb already reaching; **`Loading`** for a single record, a form scene, a whole route. A skeleton is a promise about shape, and on a recipe you have never opened that promise is a lie — both exist on purpose, and the rule is that question | `components/Skeleton.tsx`, `components/Fallback.tsx` |
| A read that FAILED with nothing cached | **`LoadError`** — one line + « Réessayer », and it drops the error TONE when the device is merely offline (that is weather, not a surprise; the board already says it once at the top). Never a bare `<p role="alert">`, never a silent empty state: an empty list where a fetch failed quietly hides real data | `components/LoadError.tsx` |
...
| Confirm a destructive delete / undo a light one | **`useConfirm`** / the undo toast — and its message says WHAT IS LOST, never a bare « Supprimer ? » (a **test** enforces it: `src/lib/confirmCopy.test.ts`, every `…Confirm` string ≥ 6 words with a consequence). Model: « Supprimer ce groupe ? Les personnes restent dans le cercle. » | `lib/confirm.tsx`, `lib/toast.tsx` (`lib/undoStack.ts`) |
...
| A call to Workers AI | **`runModel(env, model, input)`** (`_lib/runModel.ts`) — it charges the household's daily AI budget (`_lib/usage.ts`, migration 0137) and throws when it is spent, which every caller already handles as « the model did not answer ». A **test** enforces it (`usageRule.test.ts`): a raw `env.AI.run(` fails the build | …call `env.AI.run(...)` directly. Two resources here cost money per use, and a demo sandbox is a real operator session — there were thirteen unmetered call sites before this seam |
| Storing an uploaded blob in R2 | **`uploadR2Media()`** (`_lib/r2.ts`) — the per-request `maxBytes` every call site already passes, PLUS the daily aggregate no call site can see. Same guard | …`PHOTOS.put(...)` from a handler (the nightly backup is the one ALLOWED exception, with its reason) |
...
| A text field taking focus by itself | **only when the tap that revealed it asked to type** — a SectionAdd ＋, a ✏️, a day's ＋, a rename dialog whose only content is the field | …`autoFocus` a field because its screen, sheet or dialog OPENED: on a phone the keyboard takes half of what just opened (opening a person to read their address put the keyboard over it). « The form opens ready to type » is not a reason. `src/lib/autofocus.test.ts` fails the build on a new site without one |
...
| Day arithmetic on a local midnight | **`addLocalDays`** (`lib/localDay.ts` / `_lib/ids.ts`) | …`± 86400`: DST days are 23/25 h long, so a fixed day off a local midnight lands at 23:00/01:00 — the documented fixed-86400 trap, which also made two e2e specs red twice a year, and which bit the timezone test written to prove the trap (two midnights three zones apart are 21 h apart, not 24) |
| WHICH zone a day belongs to | nothing — **`authed()` already put the household's zone in scope** (`_lib/tz.ts`, migration 0135), and every helper in `ids.ts` reads it as its default | …pass `'America/Toronto'`, or read `HOUSEHOLD_TZ` expecting a constant: it is the CURRENT request's zone now. Outside a request (the cron, a unit test) it answers America/Toronto |
