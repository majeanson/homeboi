...
| **Recipe collections**       | Browse recipes grouped by an existing **tag** (no new table); parent = a single-select cover-card **toggle group** (`CollectionPicker`) inside the recipe book, alongside the tag chips (no sub-tabs) + toddler hear-first picker (`KidCollections`).                    |
