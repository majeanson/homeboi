...
| Till (`/liste/cashier`) · show Flipp / sync the list to Flipp / words to Flipp / empty the Flipp list | ✅ four doors on the grid row (+ the card): **« Montrer Flipp »** → the **FlippPager**, Flipp's own item page FRAMED over the till (« Précédent / Suivant », ← →, « Terminé »; each pick shown wears the tile ✓; « Ouvrir dans Flipp » = the same DIRECT `/fr-ca/item/<id>` in a real tab — never `/action`, which the Flipp iOS app claims and opens on its empty list). **« Ma liste → Flipp »** — the ONE sync door: a link to flipp.com/liste_dachats`#bb=<base64url list>` (Safari itself on iOS), where the served bookmarklet `public/flipp-paste.js` reads the list first and draws its own SHEET — « Remplacer ma liste Flipp » (signed in: a `delete` per existing row + every payload row in ONE account PUT, box-less clippings completed from Flipp's item endpoint; signed out: the local list) · « Ajouter » · « Rapporter Flipp → Babillard » · « Annuler »; the clipboard gets the list too as the menu's fallback; after a write, a language-mismatch sheet points at Réglages ▸ Magasinage ▸ « Langue de ton app Flipp ». **« Envoyer à Flipp »** — words only, no bookmark: `flipp.com/action?command=add_text_to_list&texts=…`, the Flipp app's universal link; primary only when no pick is live. **« Vider ma liste Flipp »** — the same door with a clear request; the bookmark confirms ON flipp.com naming what is lost (no undo — the confirm IS the tier). All four need the household postal; the per-item « Ajouter à Flipp · n de N » loop retired 2026-09-11 | — | ✅ the card IS the peek | — | liste▸shop (postal · « Langue de ton app Flipp » · the « Ma liste Flipp » how-to) | n/a (external) | ✅ |
