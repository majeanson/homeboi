...
| Till (`/liste/cashier`) · show Flipp / add to Flipp list / see Flipp list | ✅ card « Montrer Flipp » AND grid « Montrer Flipp » → the **FlippPager**: Flipp's own item page FRAMED over the till (flipp.com allows framing — probed 2026-09-11), « Précédent / Suivant » (+ ← → keys) through every live pick, « Terminé » on the last; each pick it lands on wears the tile ✓; the grid door resumes at the first not yet shown; the bar carries « Ouvrir dans Flipp » (the same DIRECT `/fr-ca/item/<id>` URL in a real tab — never `/action`, which the Flipp iOS app claims and opens on its EMPTY list). Adding stays outside the frame (partitioned storage): grid stepper « Ajouter à Flipp · n de N » (opens the NEXT pick's Flipp page in a real tab) + « Envoyer à Flipp » (ONE link: `flipp.com/action?command=add_text_to_list&texts=…` adds every unchecked line as a typed item and opens the Flipp app on a phone that has it — their universal-link path) + « Ma liste → Flipp » (a LINK: opens flipp.com's list page — Safari itself on iOS — with the whole list in the URL hash `#bb=<base64url>`, where the bookmark reads it first: no copy, no paste permission; the clipboard still gets it as the fallback; the served bookmarklet `public/flipp-paste.js`; SIGNED IN on flipp.com the bookmark writes straight into the ACCOUNT list by Flipp's own join PUT — their login-time merge is the only other time that happens, which is why a signed-in paste used to show the old content; signed out it fills the local list) — the paste draws its own SHEET on flipp.com with named buttons: « Remplacer ma liste Flipp » (a `delete` per existing row + every payload row in one PUT) · « Ajouter à ma liste Flipp » · « Rapporter Flipp → Babillard » · « Annuler »; with no list on the clipboard: « Coller un texte… » · « Rapporter » · « Vider ma liste Flipp » · « Annuler » (`confirm()` cannot relabel its buttons; code on their page can draw its own — native dialogs remain only as the fallback where the page has no body); so « Vider » then « Coller » is one run + « Vider ma liste Flipp » (copies a CLEAR request; the bookmark confirms ON flipp.com, naming what is lost — « Vider ta liste Flipp ? Tout ce qui s'y trouve — rabais et articles — sera retiré. » — then a `delete` op per row of the account list, or an empty local list; « vider » typed in the bookmark's paste box does the same; no undo — the confirm IS the tier) the bookmark is a loader whose address never changes — live clippings as clippings, every other unchecked row as a typed item; an ENDED deal gets no « Montrer Flipp » and rides as a typed item — `lib/flippList`, which writes Flipp's own list storage from flipp.com's origin; the only way in, probed 2026-09-10); all three link-outs need the household postal | — | ✅ the card IS the peek | — | liste▸shop (postal; the « Ma liste Flipp » bookmark card) | n/a (external); the step bookmark is device-local, « Reprendre du début » clears it | ✅ |
