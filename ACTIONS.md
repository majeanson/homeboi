...
| List · bring back from Flipp | ✅ `/liste#flipp=…` — the bookmark's second answer opens Babillard with Flipp's list; a confirm names the consequence (« rien n'est retiré »), then `stageDeal`/`ensureListLine`/`checkListLine` through the picks seam | — | — | — | liste▸shop (« De Flipp vers Babillard ») | confirm (a batch of adds/checks; each line then has its own undo tier) | ✅ |
| Till · refresh ended deals | ✅ « n rabais terminés · chercher ceux de cette semaine » (grid, only when some ended; `refreshEndedDeals`) | — | — | — | — | none — each line re-stages or unstages; ➖ the old deal was already dead | ✅ |
| Till (`/liste/cashier`) · show a deal to the cashier | ✅ tap a tile → the proof card → **« Montrer Flipp »** (`.bigcard__flipp`) → the **FlippPager**, Flipp's own item page FRAMED over the till (« Précédent / Suivant », ← →, « Terminé »; each pick shown wears the tile ✓; « Ouvrir dans Flipp » = the same DIRECT `/fr-ca/item/<id>` in a real tab — never `/action`, which the Flipp iOS app claims and opens on its empty list). Needs the household postal; an ENDED deal gets no door at all. **The grid-wide Flipp ROW is gone (2026-09-12)** — it carried four doors and only this one belonged at a register: the other three are LIST acts (see the row below). The pager therefore starts on the pick whose card you tapped, not on "the first not yet shown" | — | ✅ the card IS the peek | — | liste▸shop (postal · « Langue de ton app Flipp ») | n/a (external) | ✅ |
| La liste · **« Ma liste Flipp »** — send / send the deals only / send without the deals / empty | ✅ a button beside « Montrer à la caisse », opening the **FlippSheet** modal. It SAYS WHAT WILL TRAVEL before the tap (« n articles · n rabais avec sa photo », « seulement ce qui reste à acheter part », and the count of ticked lines that stay home) — the one question the old till row answered only afterwards. **« Envoyer ma liste »** → flipp.com/liste_dachats`#bb=<base64url list>` (Safari itself on iOS), where `public/flipp-paste.js` reads it and draws its own sheet — « Remplacer ma liste Flipp » · « Ajouter » · « Rapporter Flipp → Babillard » · « Annuler ». **« Les rabais seulement »** — the same payload minus its `items` half, so the week's finds land in Flipp without the whole grocery list landing with them (2026-09-12); hidden when no deal is live, since a door with an empty cargo is noise. **« Envoyer sans les rabais »** — words only, no bookmark: `flipp.com/action?command=add_text_to_list`. **« Vider ma liste Flipp »** — the same door with a clear request; the bookmark confirms ON flipp.com naming what is lost. All three need the household postal — without it the sheet shows ONE sentence and the door that fixes it, not three doors that would fail. `flippBundle` (lib/flippList) is the ONE derivation of what travels. It is handed EVERY staged deal, never the till-filtered set — « À la caisse : Non » hides a store from its own register and means nothing to Flipp (fixed 2026-09-12, guarded in flipp-sheet.spec). A deal that cannot travel AS a deal (ended, or unknown to Flipp) still travels as its words, and the sheet SAYS so with the reason rather than letting it read as « not added » | — | — | ✅ the sheet IS the surface | liste▸shop (the numbered walkthrough) | n/a (external; the clear's confirm on flipp.com IS the tier) | ✅ |

### Les notes (`components/cercle/CercleNotes.tsx`, `NotesList.tsx`, `components/virements/*`) — iOS-Notes style (2026-09-04: dropped the Simple/Avancé split), two SECTIONS since 2026-09-11 (notes · virements, `?section=`)
...
| Entente (plan) · create / edit / delete | ✅ `RowActions` ✏️ on the plan card | — | — | ✅ **two doors, and it needed both** (2026-09-14): the EmptyState's « Ajouter une entente » when none exists, **and the same-labelled `Chip` at the foot of the plan cards once one does** — everything under it already handled N ententes (the composer groups its ticked dates per plan, the memo names each, the year folds by entente), but the only create door stopped existing the moment the first agreement was written, so a household with a mortgage could not add the daycare beside it. Deliberately NOT a second ＋ tile: `notes:virements`'s ＋ is the one-tap « j'ai envoyé un virement » (every two weeks), and a chooser would tax that to pay for this (once a year) | `/virement/plan/:id/edit` scene | confirm (scene)⁶⁷ | ✅ |
