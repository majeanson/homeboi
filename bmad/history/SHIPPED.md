# bmad/history — what shipped, before the current session

This is `STATE.md`'s « What just shipped » section, everything older than the newest
entries. It was moved here on 2026-09-17 because the front door had become a chronicle:
4 289 lines, of which the only OPEN work started at line 3 490. Someone asking « what
should I do next? » read 81 % of the file before reaching the answer.

**Nothing was deleted.** Every entry keeps its reasoning, including the alternatives
that were rejected — that is the thing worth keeping, and the reason this folder exists
(see `README.md`). What was removed from the front door is the CHRONOLOGY: the same
decision told once when planned, once when shipped, and once when reviewed.

Git holds all of it too, with better fidelity — but a commit message is not greppable
the way a named heading is, so the narrative stays.

**Do not mine this for work.** It is finished. `STATE.md` §4 holds what is open.
...
Marc: **« keep the delete all list still — right now the copy bookmark doesn't work for
deals, it's the same clipped content »**, then **« there are shared lists, maybe we can
do something with that »**.

**The paste bug, read in their code.** `ShoppingListService` merges a LOCAL list into
the account in exactly one place: `joinLocalList`, fired by the `flipp-auth` event with
`action: 'login'` — the path they wrote for "added while logged out, then signed in".
Already signed in, `_loadShoppingList` rebuilds from the server (a `_delegate` proxy)
and the local list the bookmark just wrote is dropped on the floor. So the bookmark
"worked" on 2026-09-10 (signed out → paste → sign in) and "showed the same clipped
content" on 2026-09-11 (signed in → paste). Not a Flipp change; a path never taken
before.

**The fix keeps everything on flipp.com.** With a session, the bookmark now does what
their login merge does: reads the bearer Flipp keeps in its own `flipp-login` cookie
(on their origin, used from their origin, stored nowhere — the difference from « Lier
Flipp », whose whole cost was a stored third-party key), GETs the account's lists (the
first, or POSTs one), GETs its rows, and PUTs `{ commit_version, _ops }` with a `post` op
per clipping / typed item not already there — the object `SLFlyerItemClipping.createOp
('post')` builds, field for field. Signed out, the local path is unchanged. A refused
call names the step and status and writes nothing. CORS: the gateway answers a
preflight from `https://flipp.com` for PUT with authorization (checked), and a bad token
gets a clean 401. **Whether an account clipping written this way RENDERS in the app is
the open question the revert left** — their own `joinLocalList` writes the same op and
Marc's 2026-09-10 sign-in run reached the app, so the bet is on the op shape; the
answer is one signed-in paste on the phone away.

**« Vider ma liste Flipp »** — a till button that copies a CLEAR payload
(`{"v":1,"clear":true}`); the bookmark reads it (or « vider » typed in its paste box),
confirms ON flipp.com naming what is lost, then a `delete` op per row of the account
list (each echoing id + commit_version, as `createOp('delete', true)`) or an empty local
list, and lands on `/liste_dachats`. Réglages' step 4 now says why to sign in.

**Shared lists (Flipp's "family sync").** Read: an invite is generated by the owner's
account (`GET …/shopping_lists/{id}/invite`) and accepted by ANOTHER Flipp account
(`POST …/shopping_lists[/{id}]/join` with `invite_id`), which then writes the shared
list with its own bearer. So "something with that" means Babillard holding a Flipp
account of its own (signup, their terms), accepting the household's invite, and writing
from the server — the « Lier Flipp » architecture with a bot account instead of a
...
`flippClearOps`), « Annuler » on the confirm; `flippPaste.test.ts` holds the served file
to the body. `cashier.spec` covers the till row.
